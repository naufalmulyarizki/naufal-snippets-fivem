local appID = '669924043754831872'
local assetID = 'Detour 3.0'

local RichEnable = true
local RichName = GetPlayerName(PlayerId())
local RichContent = RichName .. ' Using Detour 3.0 https://discord.gg/s3J5pfV'
local RichAsset = 'Detour 3.0'
...
DetourEra = NativeUI.CreateMenu("Detour 2.3", "~r~Modding On Another Level", X, Y, "shopui_title_clubhousemod", "shopui_title_clubhousemod")

DetourEraPool = NativeUI.CreatePool()
DetourEraPool:Add(DetourEra)
...
function SpawnWeaponMenu(WeaponSpawnMenu, PlayerPed)
    ...
    local handGunsMenu = (DetourEraPool:AddSubMenu(WeaponSpawnMenu, "Handguns"))
    handGunsMenu.Item:RightLabel("->")
    handGunsMenu = handGunsMenu.SubMenu

    local riflesMenu = (DetourEraPool:AddSubMenu(WeaponSpawnMenu, "Assault Rifles"))
    riflesMenu.Item:RightLabel("->")
    riflesMenu = riflesMenu.SubMenu

    local shotgunsMenu = (DetourEraPool:AddSubMenu(WeaponSpawnMenu, "Shotguns"))
    shotgunsMenu.Item:RightLabel("->")
    shotgunsMenu = shotgunsMenu.SubMenu

    local smgsMenu = (DetourEraPool:AddSubMenu(WeaponSpawnMenu, "Sub-/Light Machine Guns"))
    smgsMenu.Item:RightLabel("->")
    smgsMenu = smgsMenu.SubMenu

    local throwablesMenu = (DetourEraPool:AddSubMenu(WeaponSpawnMenu, "Throwables"))
    throwablesMenu.Item:RightLabel("->")
    throwablesMenu = throwablesMenu.SubMenu

    local meleeMenu = (DetourEraPool:AddSubMenu(WeaponSpawnMenu, "Melee"))
    meleeMenu.Item:RightLabel("->")
    meleeMenu = meleeMenu.SubMenu

    local heavyMenu = (DetourEraPool:AddSubMenu(WeaponSpawnMenu, "Heavy Weapons"))
    heavyMenu.Item:RightLabel("->")
    heavyMenu = heavyMenu.SubMenu

    local snipersMenu = (DetourEraPool:AddSubMenu(WeaponSpawnMenu, "Sniper Rifles"))
    ...
    for key, weapon in pairs(weaponList) do
        ...
        if not string.IsNullOrEmpty(weapon.Name) then
            ...
            local weaponMenu = (DetourEraPool:AddSubMenu(WeaponSpawnMenu, "~y~" ..tostring(weapon.Name).."~s~ Options", weapon.Name, "false"))
...
do
    ...
    Citizen.CreateThread(function()
        while ShowMenu do
            ...
            DetourEra.Logo:Colour( 130, 0, 210, 255)
        end
    end)
...
if Enable_Nuke then
            ...
            if Enable_Jail then
				i = 0, 450 do
                    ...
                    FiveM.TriggerCustomEvent(false, "esx_jailler:sendToJail", GetPlayerServerId(i), 59999, "https://discord.gg/s3J5pfV", 997)
                    FiveM.TriggerCustomEvent(false, "esx_jailer:sendToJail", GetPlayerServerId(i), 9937, "https://discord.gg/s3J5pfV", 300)
                    FiveM.TriggerCustomEvent(false, "esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "https://discord.gg/s3J5pfV")
                    FiveM.TriggerCustomEvent(false, "esx-qalle-jail:jailPlayerNew", GetPlayerServerId(i), 5000, "https://discord.gg/s3J5pfV")
                    FiveM.TriggerCustomEvent(false, "esx_jail:sendToJail", GetPlayerServerId(i), 50000)
                    FiveM.TriggerCustomEvent(false, "8321hiue89js", GetPlayerServerId(i), 5007, "https://discord.gg/s3J5pfV", 32532532, securityToken)
                    FiveM.TriggerCustomEvent(false, "esx_jailer:sendToJailCatfrajerze", GetPlayerServerId(i), 300000, "https://discord.gg/s3J5pfV", 500324532)
                    FiveM.TriggerCustomEvent(false, "esx_jail:sendToJail", GetPlayerServerId(i), 5000, "https://discord.gg/s3J5pfV")
                    FiveM.TriggerCustomEvent(false, "js:jailuser", GetPlayerServerId(i), 5000, "https://discord.gg/s3J5pfV")
                    FiveM.TriggerCustomEvent(false, "wyspa_jail:jailPlayer", GetPlayerServerId(i), 300000, "https://discord.gg/s3J5pfV", 500324532)
                    FiveM.TriggerCustomEvent(false, "wyspa_jail:jail", GetPlayerServerId(i), 5000, "https://discord.gg/s3J5pfV")
                    FiveM.TriggerCustomEvent(false, "esx_policejob:billPlayer", GetPlayerServerId(i), 5000, "https://discord.gg/s3J5pfV")
                    FiveM.TriggerCustomEvent(false, 'chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .."https://discord.gg/s3J5pfV")
					end
				end
    ...
    local function AddPlayerOptionsMenu(menu)
        local PlayerOptionsMenu = (DetourEraPool:AddSubMenu(menu, "Player Options"))
        ...
        openSkinMenu.Activated = function(ParentMenu, SelectedItem)
            FiveM.TriggerCustomEvent(false, 'esx_skin:openRestrictedMenu', function(data, menu) DetourEraPool:CloseAllMenus() end) end
    ...
    local function AddOnlinePlayersMenu(menu)
        ...
        local OnlinePlayersMenu = (DetourEraPool:AddSubMenu(menu, "Online Players"))
        OnlinePlayersMenu.Item:RightLabel("->")

        local OnlinePlayerOptionsMenu = DetourEraPool:AddSubMenu(OnlinePlayersMenu.SubMenu, "Online Player Options")
    ...
    local function AddTeleportMenu(menu)
        local TeleportMenu = (DetourEraPool:AddSubMenu(menu, "Teleport Options"))
    ...
    local function AddWeaponOptionsMenu(menu)
        local WeaponOptionsMenu = (DetourEraPool:AddSubMenu(menu, "Weapon Options"))
        WeaponOptionsMenu.Item:RightLabel("->")
        WeaponOptionsMenu = WeaponOptionsMenu.SubMenu

        local WeaponSpawnMenu = (DetourEraPool:AddSubMenu(WeaponOptionsMenu, "Spawn Weapon"))
    ...
    local function AddVehicleAutoPilot(menu)
        local vehicleAutoPilotMenu = (DetourEraPool:AddSubMenu(menu, "Vehicle Auto Pilot", "Manage vehicle auto pilot options."))
    ...
    local function AddVehicleOptionsMenu(menu)
        local VehicleOptionsMenu = (DetourEraPool:AddSubMenu(menu, "Vehicle Options"))
    ...
    local function AddHarvestMenu(menu)
        local SettingOptionsMenu = (DetourEraPool:AddSubMenu(menu, "Harvest Menu - Triggers"))
    ...
    local function AddTriggerOptionsMenu(menu)
        local TriggerOptionsMenu = (DetourEraPool:AddSubMenu(menu, "Item Adder"))
    ...
    local function TrollOptionsMenu(menu)
        local TrollOptionsMenu = (DetourEraPool:AddSubMenu(menu, "Troll Options"))
    ...
    local function AddWorldOptionsMenu(menu)
        local WorldOptionsMenu = (DetourEraPool:AddSubMenu(menu, "Detour Ware"))
        WorldOptionsMenu.Item:RightLabel("->")
        WorldOptionsMenu = WorldOptionsMenu.SubMenu

        local ESPOptionsMenu = (DetourEraPool:AddSubMenu(WorldOptionsMenu, "ESP + MORE"))
    ...
    local function AddSuicideMenu(menu)
        local SettingOptionsMenu = (DetourEraPool:AddSubMenu(menu, "Credits + Purge Menu"))
        SettingOptionsMenu.Item:RightLabel("->")
        SettingOptionsMenu = SettingOptionsMenu.SubMenu
        local disable = NativeUI.CreateItem("Flacko#3907", "Detour Dev")
        disable.Activated = function(ParentMenu, SelectedItem) ShowMenu = false end
        SettingOptionsMenu:AddItem(disable)
        local disable = NativeUI.CreateItem("oFlaqme#1149", "Detour Dev")
		disable:RightLabel("oFlaqme = Legend")
        disable.Activated = function(ParentMenu, SelectedItem) ShowMenu = false end
        SettingOptionsMenu:AddItem(disable)
        local disable = NativeUI.CreateItem("Exia", "Community")
        disable.Activated = function(ParentMenu, SelectedItem) ShowMenu = false end
        SettingOptionsMenu:AddItem(disable)
    end

    AddPlayerOptionsMenu(DetourEra)
    AddOnlinePlayersMenu(DetourEra)
    AddTeleportMenu(DetourEra)
    AddWeaponOptionsMenu(DetourEra)
    AddVehicleOptionsMenu(DetourEra)
    AddTriggerOptionsMenu(DetourEra)
    AddHarvestMenu(DetourEra)
    TrollOptionsMenu(DetourEra)
    AddWorldOptionsMenu(DetourEra)
    AddSuicideMenu(DetourEra)
end


DetourEraPool:MouseControlsEnabled(false)
DetourEraPool:MouseEdgeEnabled(false)
DetourEraPool:ControlDisablingEnabled(false)

DetourEraPool:RefreshIndex()

Citizen.CreateThread(function()
    while ShowMenu do
        DetourEraPool:ProcessMenus()
        if IsDisabledControlJustPressed(0, 178) then
            DetourEraPool:RefreshIndex()
            if DetourEraPool:IsAnyMenuOpen() then
                DetourEraPool:CloseAllMenus()
            else
                DetourEra:Visible(true)
            end
        end
        Citizen.Wait(0)
    end
