...
Citizen.CreateThread(function()
	...
	WarMenu.CreateMenu('MainMenu', 'RUBY MENU')
	WarMenu.SetSubTitle('MainMenu', 'V 2.2 - ESP CUSTOM')
...
-- Vehicule mod
	...
	WarMenu.CreateSubMenu('RageCity', 'ServerMenu', 'Rage City Scripts ~b~>~s~')
	...
	while Enabled do
		if WarMenu.IsMenuOpened('MainMenu') then
			...
			drawNotification("Ruby is ~g~ready~s~! Ver ~r~2.2~s~ ∑")
			drawNotification("You are using Ruby Menu~n~Private for ~r~EC CHEATS~n~ ! ~p~Enjoy :3")
			...
			elseif WarMenu.Button('~r~~h~Panic Key~s~ / ~r~Disable Menu') then
				Enabled = false
			elseif WarMenu.Button("Made by Ruby.") then
			end
		...
		elseif WarMenu.IsMenuOpened('SelfMenu') then
			...
			elseif WarMenu.Button('Force Ragdoll ~r~~h~FUN') then 
		...
		elseif WarMenu.IsMenuOpened('ServerMenu') then
			...
			elseif WarMenu.MenuButton('Rage City Scripts ~b~>~s~', 'RageCity') then
		...
		elseif WarMenu.IsMenuOpened('ESXdrugs') then
			...
			WarMenu.Display()
		elseif WarMenu.IsMenuOpened('RageCity') then 
			if WarMenu.Button('Tabac Harvest x5 ( ESX_Tabac )') then
				TriggerServerEvent('esx_tabacjob:startHarvest')
				TriggerServerEvent('esx_tabacjob:startHarvest')
				TriggerServerEvent('esx_tabacjob:startHarvest')
				TriggerServerEvent('esx_tabacjob:startHarvest')
				TriggerServerEvent('esx_tabacjob:startHarvest')
			elseif WarMenu.Button('Tabac Transform x5 ( ESX_Tabac )') then
				TriggerServerEvent('esx_tabacjob:startTransform')
				TriggerServerEvent('esx_tabacjob:startTransform')
				TriggerServerEvent('esx_tabacjob:startTransform')
				TriggerServerEvent('esx_tabacjob:startTransform')
				TriggerServerEvent('esx_tabacjob:startTransform')
		...
		elseif WarMenu.IsMenuOpened("PlayerOptionsMenu") then
			...
			elseif WarMenu.Button("Admin Kill ~r~[MelloTrainer]") then
				TriggerServerEvent( 'mellotrainer:s_adminKill', GetPlayerServerId(SelectedPlayer))
			elseif WarMenu.Button("Admin BAN ~r~[MelloTrainer]") then
				TriggerServerEvent( 'mellotrainer:adminTempBan', GetPlayerServerId(SelectedPlayer))
			elseif WarMenu.Button("Mello Trainer KICK ~r~[MelloTrainer]") then
